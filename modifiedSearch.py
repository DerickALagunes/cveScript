...
argParser.add_argument('-d', type=str, help='D = search for new things after an especific date yyyy-MM-ddTHH:mm:ss.sss-Z')
argParser.add_argument('-lvl', type=str, help='Especify warning level 0-10')
...
if dDate:
    ...
    dDate = "1999-01-01T00:00:00.000-04:00"
...
# Search Product (best to use CPE notation, e.g. cisco:ios:12.2) && Date
if vSearch:
    for item in db.getCveNewerThan(dDate, vSearch, dLvl):
        if csvOutput:
            refs = []
            ...
            if not namelookup:
                csvoutput.writerow([item['id'], item['Published'], item['Modified'], item['cvss'], item['summary'], refs])
            else:
                csvoutput.writerow([item['id'], item['Published'], item['Modified'], item['cvss'], item['summary'], refs, nl])
        elif htmlOutput:
        ...
        # representation
            ...
            c = SubElement(r, 'Modified')
            c.text = item['Modified']
            c = SubElement(r, 'cvss')
